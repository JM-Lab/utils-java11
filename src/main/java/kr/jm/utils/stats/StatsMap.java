...
public class StatsMap implements Map<StatsField, Number> {
    ...
    public StatsMap(List<Number> numberList) {
        this(new NumberSummaryStatistics(numberList).getStatsFieldMap());
    }
    ...
    public boolean equals(Object o) {
        ...
        StatsMap statsMap = (StatsMap) o;
        return Objects.equals(statsFieldNumberMap, statsMap.statsFieldNumberMap);
    }

    @Override
    public int hashCode() {
        return Objects.hash(statsFieldNumberMap);
    }
    ...
    public StatsMap merge(StatsMap statsMap) {
        synchronized (statsFieldNumberMap) {
            statsFieldNumberMap
                    .put(count, statsFieldNumberMap.get(count).longValue() + statsMap.get(count).longValue());
            statsFieldNumberMap.put(sum, statsFieldNumberMap.get(sum).doubleValue() + statsMap.get(sum).doubleValue());
            statsFieldNumberMap.put(min,
                    Math.min(statsFieldNumberMap.get(min).doubleValue(), statsMap.get(min).doubleValue()));
            statsFieldNumberMap.put(max,
                    Math.max(statsFieldNumberMap.get(max).doubleValue(), statsMap.get(max).doubleValue()));
            statsFieldNumberMap.put(avg,
                    statsFieldNumberMap.get(sum).doubleValue() / statsFieldNumberMap.get(count).doubleValue());
            return this;
        }
    }
