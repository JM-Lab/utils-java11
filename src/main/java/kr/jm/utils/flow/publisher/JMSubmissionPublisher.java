...
public class JMSubmissionPublisher<T> extends SubmissionPublisher<T> implements JMPublisherInterface<T> {
    ...
    public JMSubmissionPublisher(int publishers, int maxBufferCapacity, long waitingMillis) {
        super(publishers > 0 ? JMThread.newThreadPool(publishers) : ForkJoinPool.commonPool(), maxBufferCapacity);
    ...
    public int submit(T item) {
        ...
        try {
            ...
            return JMException.handleExceptionAndReturn(log, e, "submit", () -> 0, item);
        }
    }
    ...
    public String toString() {
        return "JMSubmissionPublisher{" + "publishers=" + publishers + ", maxBufferCapacity=" + maxBufferCapacity +
