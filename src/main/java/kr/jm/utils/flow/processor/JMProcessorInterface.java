 ...
 */
public interface JMProcessorInterface<T, R> extends Flow.Processor<T, R>, JMPublisherInterface<R> {
    /**
     ...
     * @return the s
     */
    default <O, S extends JMProcessorInterface<R, O>> S subscribeAndReturnProcessor(S returnTransformProcessor) {
     ...
     */
    default <O> JMProcessorInterface<R, O> subscribeAndReturnProcessor(Function<R, O> transformerFunction) {
        return subscribeAndReturnProcessor(JMProcessorBuilder.build(transformerFunction));
    }
    ...
    @Override
    default JMProcessorInterface<T, R> subscribeWith(Flow.Subscriber<R>... subscribers) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    default JMProcessorInterface<T, R> consumeWith(Consumer<R>... consumers) {
